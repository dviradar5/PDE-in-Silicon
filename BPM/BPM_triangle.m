...
title(sprintf('Pulso Gaussiano Original %de^{-(x/%g)^2}',A,W0));
...
for j = 1:1:num_samples
    if (x(j) >= xmin) && (x(j) <= 0)
        n(j) = m1*(x(j)-xmin) + nmin;
    elseif (x(j) >= 0) && (x(j) <= xmax)
        n(j) = m2*x(j) + nmax;
    else
        n(j) = nmin;
    end
...
surf(x1,y1,z1,'FaceColor','interp','edgecolor','none','FaceLighting','phong');
colormap copper;
camlight right;
grid on;
view (30,45);
